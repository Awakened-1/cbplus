...
function generalStuff()
{
  ...
  else if(document.location.pathname.toString()
    .length < 2)
  {
...
function camsSite()
{
  ...
  frame.src = 'https://chaturbate.com/tours/3/?p=1&c=200&playerID=' +
    playerID
  frame.style.flex = '1'
  ...
  hideMenu.innerHTML =
    `<a style="color: gold;">        HIDE/SHOW LIST</a>`;
  ...
  document.getElementById("nav")
    .appendChild(hideMenu);

  //  let storageLoadMenu = document.createElement("li");
  //  storageLoadMenu.innerHTML = `<a style="color: gold;">        Load Storage</a>`;
  //  storageLoadMenu.style.cursor = 'pointer'
  //  storageLoadMenu.onclick = function () { addToLocalStorage() };
  //  document.getElementById("nav").appendChild(storageLoadMenu);
...
function blackSite()
{
  ...
  for(var i = 0; i < len; i++)
  {
    ...
    li.onclick = function()
    {
      if(confirm('Are you sure you want to delete ' + this.innerHTML
          .split(",")[0] + ' from blacklist?'))
      {
        GM_deleteValue('cbplus_blacklist_' + this.innerHTML.split(
          ",")[0]);
...
function whiteSite()
{
  ...
  for(var i = 0; i < len; i++)
  {
    ...
    li.onclick = function()
    {
      ...
      GM_deleteValue('cbplus_whitelist_' + this.innerHTML.split(
        ",")[0]);
      //console.log(this.innerHTML + " is not longer on BLACKLIST");
      this.remove();
...
function toursPage()
{
  ...
  playerID = playerID.substring(playerID.indexOf("playerID"))
    .split("&")[0].split("=")[1]
  globals.chat = new BroadcastChannel(playerID)
}
...
function readMessage(msg)
{
  ...
  let check = document.body.querySelectorAll(
    "div#mainDiv > div[name=\"" + cmd[1] + "\"]")
  let wins = document.querySelectorAll("div#mainDiv > div.free")
  if(wins.length == 0 && !check.length) wins = addCamPlace()
  if(cmd[0] == "watch" && cmd[1].length > 0 && wins.length > 0 && !
    check.length)
  {
    globals.http.open('GET', `https://chaturbate.com/${cmd[1]}`, true)
    globals.http.setRequestHeader("Content-type",
      "application/x-www-form-urlencoded")
    globals.http.onload = function()
    {
      addCam(globals.http.responseText, wins[0], cmd[1])
    }
    globals.http.send()
  }
  else if(check.length)
  {
    console.log("already watching " + cmd[1] + "!")
  }
  else
  {
...
function cleanPage()
{
  ...
  if(content.length > 0)
    if(content[0].style.padding != Element.style.padding) content[0]
      .style.padding = Element.style.padding;

  Element.style.margin = "0 10px";
  var c1Main = document.getElementsByClassName(
    'c-1 endless_page_template');
  if(c1Main.length > 0)
    if(c1Main[0].style.margin != Element.style.margin) c1Main[0].style
      .margin = Element.style.margin;

  var c1 = document.getElementsByClassName("c-1");
  if(c1.length > 0)
    if(c1[0].style.margin != Element.style.margin) c1[0].style
      .margin = Element.style.margin;

  var blogPosts = document.getElementsByClassName(
    'c-1 featured_blog_posts');
...
function addCamPlace()
{
  ...
  let len = main.querySelectorAll('div.cam')
    .length
  let loops = 0
...
function addMiniButtons()
{
  ...
  {
    let name = rooms[i].querySelector('a')
      .getAttribute('href')
      .slice(1, -1)
    ...
    let tmpName = rooms[i].querySelector('div.title a')
      .getAttribute("href")
      .slice(1, -1)
    rooms[i].querySelector('a')
      .removeAttribute("href")
    rooms[i].style.cursor = 'pointer'
    rooms[i].setAttribute('style', 'margin: 6px 6px')
    rooms[i].querySelector('a')
      .setAttribute("name", tmpName)
    rooms[i].querySelector('a')
      .onclick = function()
      {
    ...
    blockButton.onclick = function()
    {
      ...
      if(GM_getValue(`cbplus_whitelist_${name}`))
      {
        ...
        cam.querySelector('.details')
          .setAttribute("style",
            "background-color: rgb(255, 255, 255)")
        return;
      }
      //if (confirm('Are you sure you want to add ' + name + ' to blacklist?')) {
      let gender = cam.querySelector('div.title span')
        .className.substr(-1)
      let age = cam.querySelector('div.title span')
        .innerHTML;
      let value = gender + " " + age + " added: " + new Date()
        .toLocaleString();
    ...
    addButton.onclick = function()
    {
      let cam = this.parentNode.parentNode
      let name = this.getAttribute("name")
      let gender = cam.querySelector('div.title span + span')
        .className.substr(-1)
      let age = cam.querySelector('div.title span')
        .innerHTML;
      let value = gender + " " + age + " added: " + new Date()
    ...
    let tempViewers = rooms[i].querySelector(
        'div.details > ul.sub-info > li.cams')
      .textContent;
    let viewers = Number(tempViewers.slice(tempViewers.indexOf(',') +
        1, tempViewers.indexOf(' view'))
      .trim());
    //************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%
    //************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%
    //************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%************$$$$$$$%%%%%%%%%%%%%%%%%%%
    //  if (viewers < 5 || viewers > 20) rooms[i].style.display = "none";
    if((globals.path == globals.privatePath) || (globals.path ==
        globals.newbiePath))
    {
      rooms[i].style.display = "block";
    }
    else if(viewers < 5 && globals.path != globals.privatePath) rooms[
      i].style.display = "none";

    let gender = rooms[i].querySelector(
        'div.age_gender_container span + span')
      .className.substr(-1);
    let age = rooms[i].querySelector('div.title span')
      .innerHTML;
    let tags = rooms[i].querySelector('div.details ul.subject li')
      .innerText.toString()
      .toLowerCase();
    tags += rooms[i].textContent.toString()
      .toLowerCase();
    ...
    //  console.log(tags);
    if((tags.indexOf("#asian") > 0) || (tags.indexOf("#pregnant") >
      0))
    {
      rooms[i].style.display = "none";
    }
    ...
    if(rooms[i].querySelector(
        'li > div.thumbnail_label.thumbnail_label_c_new'))
    {
      rooms[i].querySelector('.details')
        .setAttribute("style",
          "background-color: rgba(107, 165, 49, .4)");
    };
    if(GM_getValue(`cbplus_whitelist_${name}`) != undefined)
    {
      rooms[i].querySelector('.details')
    ...
    if(Number(age) >= ageLimit && Number(age) < 65) rooms[i].style
      .display = "none"; //
    if(gender == 'm' || gender == 's' || gender == 'c') rooms[i].style
      .display = "none";
...
function addTabs()
{
  ...
  if(sub_nav)
  {
    document.querySelector("div.nav-bar")
      .style.height = "auto"
    // cams Tab
    var camsTab = document.createElement("li");
    camsTab.innerHTML =
      `<a style="color: gold;" href=\"/cams-cbplus/\">CBPLUS</a>`;
...
function addCam(resp, div, model)
{
  ...
  {
    stream = resp.substring(pos1, pos2)
      .replace(/\\u002D/g, '-')
  }
  else
  {
  ...
  let poster = 'https://cbjpeg.stream.highwebmedia.com/stream?room=' +
    model + '&f=' + Math.random()
...
function refreshCam(div)
{
  ...
  globals.http.open('GET', `https://chaturbate.com/${model_name}`,
    true)
  globals.http.setRequestHeader("Content-type",
    "application/x-www-form-urlencoded")
  globals.http.onload = function()
  {
    addCam(globals.http.responseText, div, model_name)
  }
  globals.http.send()
}
...
function plusButton()
{
  ...
  b.addEventListener('click', e =>
  {
    ...
    if(user_data !== null)
    {
      ...
      if(user_data.includes('/') || user_data.includes(
          'chaturbate.com'))
      {
      ...
      globals.http.open('GET',
        `https://chaturbate.com/${user_data}`, true)
      globals.http.setRequestHeader("Content-type",
        "application/x-www-form-urlencoded")
      globals.http.onload = function()
      {
        addCam(globals.http.responseText, e.path[1], user_data)
      }
      globals.http.send()
    }
  })
...
function topButtons(name)
{
  ...
  h.addEventListener('click', e =>
  {
    ...
    GM_setValue(`cbplus_whitelist_${name}`, new Date()
      .toLocaleString());
  })
...
{
  window.open(
    `https://chaturbate.com/api/chatvideocontext/${name}/ `,
    "_blank",
    'scrollbars=yes,menubar=yes, toolbar=yes,location=yes,addressbar=yes,width=780px,height=670px'
    )
}
